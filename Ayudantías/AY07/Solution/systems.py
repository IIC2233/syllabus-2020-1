...
class Mailer(QObject):

    send_response_signal = pyqtSignal(dict)
    ...
    def send_mail(self, sender, receiver, subject, content):
        ...
        # Se verifica que el destinatario se haya ingresado y sea válido.
            ...
            with open(os.path.join("Data", "actions.csv"),
                ...
                writer = csv.DictWriter(file, fieldnames=["Sender", "Receiver",
                                        "Subject", "Content", "Date"])
                writer.writerow({"Sender": sender, "Receiver": receiver, "Subject": subject,
                                 "Content": content, "Date": date})

            return (200, notificacion)

    # Definir manejador de respuesta
    def window_handler(self, data):
        sender = data["from"].strip()
        receiver = data["to"].strip()
        subject = data["subject"].strip()
        content = data["content"]

        status_code, msg = self.send_mail(sender, receiver, subject, content)

        data = {"status": status_code, "notification": msg}
        self.send_response_signal.emit(data)
